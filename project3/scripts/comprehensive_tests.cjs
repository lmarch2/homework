...
async function runComprehensiveTests() {
    ...
    for (let i = 0; i < testCases.length; i++) {
        ...
        try {
            ...
            execSync(`node build/poseidon2_hash_js/generate_witness.cjs build/poseidon2_hash_js/poseidon2_hash.wasm ${inputFile} ${witnessFile}`,
                { stdio: 'inherit' });
            ...
            console.log(`证明验证: ${success ? '通过 OK' : '失败 ✗'}`);
            console.log();
    ...
    console.log(`唯一性检查: ${hashes.length === uniqueHashes.length ? '通过 OK' : '失败 ✗'}`);
    console.log();

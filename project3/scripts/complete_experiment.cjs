...
async function runCompleteExperiment() {
    ...
    if (!allFilesExist) {
        console.log("\nFAILED Setup incomplete. Please run setup first.");
        return;
    }
    ...
    console.log(`   Proof verified: ${result1.verified ? 'OK' : 'FAILED'}`);
    ...
    console.log(`   Proof verified: ${result2.verified ? 'OK' : 'FAILED'}`);
    ...
    console.log(`   Proof verified: ${result3.verified ? 'OK' : 'FAILED'}`);
    ...
    console.log(`   Hash uniqueness: ${new Set([result1.hash, result2.hash, result3.hash]).size === 3 ? 'OK' : 'FAILED'}`);

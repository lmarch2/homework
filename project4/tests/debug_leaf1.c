...
int main()
{
    ...
    // 同时验证期望的叶子1哈希
    uint8_t expected_leaf1[32];
    merkle_compute_leaf_hash((uint8_t *)"leaf_3", strlen("leaf_3"), expected_leaf1);
    print_hash(expected_leaf1, "期望叶子1哈希");

    printf("叶子哈希匹配: %s\n", memcmp(proof.leaf_hash, expected_leaf1, 32) == 0 ? "是" : "否");
